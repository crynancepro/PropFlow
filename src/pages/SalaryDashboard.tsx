...
export default function SalaryDashboard({ trades, language, currency }: SalaryDashboardProps) {
  ...
  return (
    <div className="space-y-6" id="salary-dashboard-page">
      ...
      {/* 4. Métriques complémentaires du mois */}
      <div className="bg-[#0E1116]/60 border border-white/5 rounded-2xl p-5 md:p-6 space-y-4 shadow-xl">
        <h4 className="text-xs font-black tracking-widest uppercase text-slate-300 font-mono flex items-center gap-1.5 border-b border-white/[0.04] pb-3">
          <Scale className="w-4 h-4 text-pink-400" />
          {isFr ? `Métriques Clés & Régularité (${activePeriod.label})` : `Key Performance Indices (${activePeriod.label})`}
        </h4>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-[#0A0B0D]/50 rounded-xl p-4 border border-white/5 hover:border-white/10 transition-colors">
            <div className="text-[10px] text-slate-500 font-bold font-mono uppercase">{isFr ? "Taux de Réussite" : "Win Rate"}</div>
            <div className="text-lg font-black text-white font-mono mt-0.5">{stats.monthlyWinRate}%</div>
            <p className="text-[9px] text-slate-500 font-semibold">{stats.monthlyWins} gains / {stats.monthlyLosses} pertes</p>
          </div>

          <div className="bg-[#0A0B0D]/50 rounded-xl p-4 border border-white/5 hover:border-white/10 transition-colors">
            <div className="text-[10px] text-slate-500 font-bold font-mono uppercase">{isFr ? "Moyenne Quotidienne" : "Daily Average"}</div>
            <div className={`text-lg font-black font-mono mt-0.5 ${stats.monthlyGain >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
              {stats.totalSelectedMonthTrades > 0 
                ? formatMoney(stats.monthlyGain / 20) // Estimé sur 20 jours de trading par mois
                : '0.00 ' + currency}
            </div>
            <p className="text-[9px] text-slate-500 font-semibold">{isFr ? "Sur une base de 20j ouvrés" : "Based on 20 trading days"}</p>
          </div>

          <div className="bg-[#0A0B0D]/50 rounded-xl p-4 border border-white/5 hover:border-white/10 transition-colors">
            <div className="text-[10px] text-slate-500 font-bold font-mono uppercase">{isFr ? "Meilleure Journée" : "Best Day"}</div>
            <div className="text-lg font-black text-emerald-400 font-mono mt-0.5">
              +{stats.bestDayPnL.toLocaleString(undefined, { maximumFractionDigits: 1 })} {currency}
            </div>
            <p className="text-[9px] text-slate-500 font-semibold">{isFr ? "Gain quotidien max de la période" : "Max positive daily grouping"}</p>
          </div>

          <div className="bg-[#0A0B0D]/50 rounded-xl p-4 border border-white/5 hover:border-white/10 transition-colors">
            <div className="text-[10px] text-slate-500 font-bold font-mono uppercase">{isFr ? "Pire Journée" : "Worst Day"}</div>
            <div className="text-lg font-black text-rose-500 font-mono mt-0.5">
              {stats.worstDayPnL.toLocaleString(undefined, { maximumFractionDigits: 1 })} {currency}
            </div>
            <p className="text-[9px] text-slate-500 font-semibold">{isFr ? "Perte nominale maximale" : "Max negative daily grouping"}</p>
          </div>
        </div>
      </div>

      {/* 5. Journal des Payouts Réels & Retraits */}
      <div className="bg-[#0E1116]/60 border border-white/5 rounded-2xl p-5 md:p-6 space-y-5 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/[0.04] pb-4">
          <div className="space-y-1">
            <h4 className="text-xs font-black tracking-widest uppercase text-slate-300 font-mono flex items-center gap-1.5">
              <Wallet className="w-4 h-4 text-emerald-400" />
              {isFr ? "Journal des Retraits & Payouts Réels" : "Real Payouts & Cashouts Log"}
            </h4>
            <p className="text-[10px] text-slate-500 font-medium">
              {isFr 
                ? "Enregistrez vos retraits effectivement reçus de vos comptes Prop Firms pour suivre votre historique bancaire réel." 
                : "Record actual cashouts received from your Prop Firms accounts to track real cash flow."}
            </p>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-emerald-500/30 hover:border-emerald-500/50 text-emerald-400 text-xs font-black rounded-xl transition-all cursor-pointer hover:scale-105 active:scale-95"
          >
            {showAddPayoutForm ? (
              ...
              <>
                ...
                <span>{isFr ? "Enregistrer un Payout" : "Record a Payout"}</span>
              </>
            )}
          </button>
        </div>

        {/* Formulaire d'ajout de payout inline */}
        {showAddPayoutForm && (
          <form 
            onSubmit={(e) => {
              e.preventDefault();
              const amt = parseFloat(newPayoutAmount);
              if (isNaN(amt) || amt <= 0) return;
              const newPayoutItem = {
                id: 'payout_' + Date.now(),
                date: newPayoutDate,
                amount: amt,
                propFirm: newPayoutFirm || 'FTMO',
            ...
            className="bg-[#0A0B0D] border border-white/5 p-4 rounded-xl grid grid-cols-1 sm:grid-cols-4 gap-4 items-end animate-fadeIn"
          >
            <div className="flex flex-col">
              <label className="text-[9px] text-slate-400 font-black uppercase font-mono mb-1.5">{isFr ? "Date de réception" : "Date Received"}</label>
              <input
                type="date"
                value={newPayoutDate}
                onChange={(e) => setNewPayoutDate(e.target.value)}
                required
                className="bg-[#0E1116] border border-white/5 rounded-xl p-2.5 text-xs text-slate-200 focus:outline-none focus:border-emerald-500 font-mono"
              />
            </div>
            <div className="flex flex-col">
              <label className="text-[9px] text-slate-400 font-black uppercase font-mono mb-1.5">{isFr ? "Montant Brut (USD)" : "Gross Amount (USD)"}</label>
              <input
                type="number"
                step="0.01"
                placeholder="Ex: 5000"
            ...
            <div className="flex flex-col">
              <label className="text-[9px] text-slate-400 font-black uppercase font-mono mb-1.5">{isFr ? "Prop Firm" : "Prop Firm"}</label>
              <input
                type="text"
                placeholder="Ex: FTMO"
                value={newPayoutFirm}
                onChange={(e) => setNewPayoutFirm(e.target.value)}
                required
                className="bg-[#0E1116] border border-white/5 rounded-xl p-2.5 text-xs text-slate-200 focus:outline-none focus:border-emerald-500 font-mono"
              />
            </div>
            <div className="flex flex-col">
              <label className="text-[9px] text-slate-400 font-black uppercase font-mono mb-1.5">{isFr ? "Statut" : "Status"}</label>
              <div className="flex gap-2">
                <select
                  value={newPayoutStatus}
                  onChange={(e) => setNewPayoutStatus(e.target.value as 'PAID' | 'PENDING')}
                  className="bg-[#0E1116] border border-white/5 rounded-xl p-2.5 text-xs text-slate-200 focus:outline-none focus:border-emerald-500 font-mono flex-1 cursor-pointer font-bold"
                >
                  <option value="PAID">{isFr ? "Payé / Reçu" : "Paid / Received"}</option>
                  <option value="PENDING">{isFr ? "Demande en cours" : "Request Pending"}</option>
                </select>
                <button
                  type="submit"
                  className="bg-emerald-500 hover:bg-emerald-400 text-[#0E1116] text-xs font-black px-4 py-2.5 rounded-xl cursor-pointer transition-colors shrink-0"
                >
                  {isFr ? "Ajouter" : "Add"}
                </button>
              </div>
        ...
        {/* Tableau d'historique des Payouts */}
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/[0.04] text-[9px] font-black uppercase tracking-wider text-slate-500 font-mono">
                <th className="py-2.5 px-3">{isFr ? "Date de réception" : "Receipt Date"}</th>
                <th className="py-2.5 px-3">Prop Firm</th>
                <th className="py-2.5 px-3 text-right">{isFr ? "Montant Brut" : "Gross Amount"}</th>
                <th className="py-2.5 px-3 text-center">{isFr ? "Répartition (Split)" : "Profit Split"}</th>
                <th className="py-2.5 px-3 text-right">{isFr ? "Votre Part Net" : "Your Net Share"}</th>
            ...
            <tbody className="divide-y divide-white/[0.02] text-xs font-mono">
              {payouts.length > 0 ? (
                payouts.map(payout => {
                  const netShare = (payout.amount * profitSplit) / 100;
                  return (
                    <tr key={payout.id} className="hover:bg-white/[0.01] transition-all">
                      ...
                      <td className="py-3 px-3 text-right text-slate-400 font-bold">
                        {payout.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}
                      </td>
                      <td className="py-3 px-3 text-center text-slate-500 font-bold">{profitSplit}%</td>
                      <td className="py-3 px-3 text-right text-emerald-400 font-black">
                        +{netShare.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}
                      </td>
                      <td className="py-3 px-3 text-center">
                        {payout.status === 'PAID' ? (
                          <span className="inline-block px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold">
                            {isFr ? "Reçu" : "Received"}
                          </span>
                        ) : (
                          <span className="inline-block px-2.5 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-bold animate-pulse">
                      ...
                      <td className="py-3 px-3 text-center">
                        <button
                          type="button"
                          onClick={() => {
                            setPayouts(payouts.filter(x => x.id !== payout.id));
                          }}
                ...
                <tr>
                  <td colSpan={7} className="text-center py-8 text-slate-500 font-medium font-sans">
                    {isFr 
                      ? "Aucun retrait enregistré dans l'historique de votre banque pour le moment. Cliquez sur 'Enregistrer un Payout' pour simuler !" 
                      : "No bank cashouts logged yet. Click 'Record a Payout' to log your first transaction!"}
                  </td>
                </tr>
              )}
            </tbody>
